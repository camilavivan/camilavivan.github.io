...
容器在被垃圾回收时，也许仍然是有用的。例如，这些容器可能包含了对于问题诊断（trouble shooting）来说非常有用的日志和数据。强烈建议将 `maximum-dead-containers-per-container` 设置为足够大的数值，至少不能小于1，以便为每一个容器至少保留一个已停止的容器。同样的，也建议为 `maximum-dead-containers` 设置一个比较大的数值。 参考 [issue #13287(opens new window)](https://github.com/kubernetes/kubernetes/issues/13287)
