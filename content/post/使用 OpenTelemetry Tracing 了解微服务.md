...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307071701943.jpeg)
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307071704817.png)
...
- Jaeger - 一个开源的端到端分布式链路追踪系统，用于从相关系统组件中收集遥测数据并实现可视化查看。
  ...
  ![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307071705312.png)

流程分解如下：

1. 用户向信使服务发送消息。NGINX 反向代理拦截消息，并将其转发给信使并将其转发给信使服务的众多实例之一。

2. 信使服务将新消息写入其数据库。
3. 信使服务在名为 chat_queue 的 RabbitMQ 消息队列上生成一个事件，以表明消息已发送。该事件为普通事件，无具体目标。
4. 与此同时：

- 4a. 信使信使服务向发送者返回一个响应，报告该消息已成功发送。
- 4b. 通知器服务注意到 chat_queue 上的新事件，并使用该事件。

5. 通知器服务在其数据库中查看新消息接收者的通知偏好。

6. 通知器服务使用接收者首选的方法发送一个或多个通知(在本教程中，选择的方法是短信和电子邮件)。
...
3. 查看通知器服务在多长时间之后才开始处理信使服务派发的事件（延迟过长可能意味着通知器服务在读取事件队列时遇到了问题，出现事件堆积）。
...
注：本教程用到了 JavaScript SDK，因为信使和通知器服务均使用 Node.js 编写而成。您还需要设置 OTel 自动埋点特性，以便了解 OTel 提供的信息类型。本教程介绍了有关 OTel Node.js SDK 的常用信息，如欲了解更多详情，请参阅 [OTel 文档]：<https://opentelemetry.io/docs/instrumentation/js/getting-started/nodejs/>。

### 设置

1. 开启终端会话。

2. 在主目录下，创建 microservices-march 目录，并将本教程会用到的 GitHub 代码库复制到其中。（您也可以使用其他目录名称，相应修改指令即可）。

>  注：本教程中省略了 Linux 命令行提示符，以便您将命令复制和粘贴到终端。波浪符 (**~**) 表示您的主目录。
...
4. 为信使服务启动 PostgreSQL 数据库：

```bash
docker compose up -d
```

1. 创建数据库模式和表格，并插入一些种子数据：
...
> 注：OTel 的不同之处在于，其 JavaScript SDK 被分解成了多个微小部分。因此，在本教程的基本示例中，您将需要多安装几个安装包。如欲了解除本教程中所列以外还需要哪些安装包才能完成监测任务，请仔细阅读（强推）OTel [入门指南]：https://opentelemetry.io/docs/instrumentation/js/getting-started 并查看 OTel [GitHub 代码库]：https://github.com/open-telemetry/opentelemetry-js#packages。

1. 新建一个名为 tracing.mjs 的文件，添加 OTel 链路追踪的设置和配置代码：

```bash
touch tracing.mjs
```

1. 在您常用的文本编辑器中，打开 tracing.mjs 并添加下列代码：
...
1. 从 OTel SDK 中导入所需的函数和对象。

2. 新建 NodeSDK 实例并对其进行配置，以便：
...
5. 在浏览器中，访问 Jaeger 用户界面：http://localhost:16686，并验证 OTLP 导出器是否按预期运行。在标题栏中点击 Search（搜索），从 Service（服务）字段的下拉菜单中选择名称以 unknown_service 开头的服务。点击 Find Traces（查找追踪）按钮：

![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307071721624.png)

6. 点击窗口右侧的链路追踪，以显示其中的 span 列表。每个 span 都描述了作为链路追踪的一部分运行的各项操作，有时涉及多个服务。截图中的 jsonParser span 显示了运行信使服务的请求处理代码的 jsonParser 部分所用的时长。

![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307071723413.png)

7. 正如第五步中所述，OTel SDK 导出的服务名称 (unknown_service) 毫无意义。要想解决这个问题，在信使终端按下 Ctrl+c 来停止信使服务。 然后再安装几个 Node.js 包：
...
11. 确认一个名为 messenger（信使）的新服务出现在浏览器的 Jaeger 用户界面中（这可能需要几秒钟的时间，而且您可能需要刷新 Jaeger 用户界面）：
    ...
    ![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100810313.png)

12. 从 Service 下拉菜单中选择 messenger，然后点击 Find Traces 按钮，即可查看由信使服务生成的所有最新链路追踪（截图显示了 20 条信息中的两条最新信息）：

![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100811164.png)

13. 点击一个追踪，以显示其中的 span。每个 span 都被正确地标记为源自于信使服务：

![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100811699.png)
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100815289.png)

### 配置 NGINX 的 OTEL 埋点

对于 NGINX，您需要手动设置链路追踪，而不是使用 OTel 自动埋点方法。目前，使用 OTel 对 NGINX 进行监测的最常见方式是使用[用 C 语言编写的模块]：`https://github.com/open-telemetry/opentelemetry-cpp-contrib/tree/main/instrumentation/otel-webserver-module#nginx-webserver-module`。第三方模块是 NGINX 生态系统的一个重要组成部分，但需要进行一些设置。本教程向您展示了如何进行这些设置。有关背景信息，请参阅我们的博文《为 NGINX 和 NGINX Plus 编译第三方动态模块》。

1. 开启新的终端会话（NGINX 终端），将目录更改为信使代码库的根目录，并新建一个名为 load-balancer 的目录及三个名为 Dockerfile、nginx.conf 和 
...
2. 在您常用的文本编辑器中，打开 Dockerfile 并添加以下内容（注释解释了每一行代码的功能，即便您不完全理解这些注释，也能构建和运行 Docker 容器）：
...
1. 打开 opentelemetry_module.conf 并添加以下内容：
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100821177.png)

## 挑战 3：学习解读 OTel 链路追踪

在架构和用户流中，我们概述了用户流的各个阶段，现在简单回顾一下：

1. 一位用户通过向另一位用户发送消息开始对话。
2. NGINX 反向代理拦截消息并将其转发给信使服务。
3. 该信使服务将消息写入其数据库，然后通过 RabbitMQ 派发事件。
4. 通知器服务使用该事件，查询接收者（第二位用户）的通知偏好，并通过首选方法向接收者发送通知。
实施遥测的目标是：

1. 了解一个请求在新的消息流中所经历的所有步骤。
2. 确认在正常情况下消息流可在五秒钟内端到端执行完毕。
3. 查看通知器服务在多长时间之后才开始处理信使服务派发的事件。
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100822153.png)
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100823010.png)
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100825791.png)
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100826997.png)
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100827801.png)
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100828438.png)
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100829140.png)
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100831081.png)
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100833645.png)
...
1. 获取 tracer，它是一个全局对象，用于与 OTel 链路追踪进行交互。
2. 开启一个名为 notification.send_all 的新的父 span，并设置 user_id 属性以识别消息的发送者。
3. 进入一个循环，其中列举了接收者的通知偏好，并在 notification.send_all 下新建了一个名为 notification.send 的子 span。每个通知都会生成一个新 span。
4. 为子 span 设置更多的属性：
- notification_type——短信或电子邮件之一
- user_id——通知接收用户的 ID
5. 依次关闭每个子 notification.send。
6. 关闭父 notification.send_all。
拥有父 span 可确保每个“发送通知”操作都会被报告，即使没有发现用户的通知偏好。

3. 在通知器终端，按下 Ctrl+c 来停止通知器服务。然后重启它：
...
![图片](https://gh-proxy.com/https://raw.githubusercontent.com/Ghostpanter/tuchuang/main/img/202307100838613.png)
...
3. 通过在信使和通知器这两个终端运行这些命令，停止并重启信使和通知器服务：
...
7. 在控制台中链路追踪 ID 是匹配的，但最后您可以将其与 Jaeger 用户界面中的链路追踪 ID 进行比较。在相关链路追踪 ID 端点（您的端点会有所不同，但在本例中是 http://localhost:16686/trace/29377a9b546c50be629c8e64409bbfb5），打开用户界面，查看整个链路追踪。Jaeger 链路追踪确认：
...
您在一个 NGINX 反向代理和两个 Node.js 服务中设置了 OTel 埋点。
您以严谨的眼光审视了 OTel 自动埋点提供的数据，并增添了一些缺失的遥测数据，以实现 OTel 实验的目标：
您在不直接更改任何应用代码的情况下，对特定请求在消息传递系统中的经历获得了清晰的视图。
...
[使用 OpenTelemetry Tracing 了解您的微服务 (qq.com)](https://mp.weixin.qq.com/s/lat-YIm9KaOc7gZZZscBaw))
